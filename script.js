...
const games = [
  ...
  },
  {
    ...
    players: "单人",
    platform: "网页 / 手机",
    description: "跟着节拍在高亮窗口内点击，连续命中会延长浪潮。",
    features: ["节拍判定", "连击加分", "窗口缩放"],
    mode: "rhythm",
  ...
  },
  {
    ...
    players: "单人",
    platform: "网页 / PC",
    description: "在倒计时内快速输入出现的关键词，越快得分越高。",
    features: ["快速打字", "限时挑战", "词条刷新"],
    mode: "typing",
  ...
  },
  {
    ...
    players: "单人",
    platform: "网页 / 手机",
    description: "记住系统给出的颜色顺序并完整复现。",
    features: ["顺序记忆", "逐轮加长", "短时反应"],
    mode: "sequence",
  ...
  },
  {
    ...
    players: "单人",
    platform: "网页 / PC",
    description: "提前选择防线位置，阻止敌方穿过网格终点。",
    features: ["线路防守", "格子部署", "回合压制"],
    mode: "defense",
  ...
  },
  {
    ...
    players: "单人",
    platform: "网页 / 手机",
    description: "翻转重力把方块送进停靠港，别让它撞上障碍。",
    features: ["重力翻转", "障碍规避", "终点停靠"],
    mode: "gravity",
...
function playMaze(stage) {
  ...
  const move = (direction) => {
    ...
    if (next.row < 0 || next.row >= layout.length || next.col < 0 || next.col >= layout[0].length || layout[next.row][next.col] === 1) {
...
function playRhythm(stage) {
  ...
  stage.innerHTML = `
    <div class="play-stage">
      <div class="status-line">
        ...
        <span class="status-pill">失误<strong id="rhythm-miss">${misses}</strong></span>
      </div>
      <div class="rhythm-track">
        <div class="rhythm-zone"></div>
        <div class="rhythm-pointer" id="rhythm-pointer"></div>
      </div>
      <div class="stage-toolbar">
        <button class="button primary" id="rhythm-hit" type="button">跟拍击中</button>
        <span class="stage-note" id="rhythm-note">让指针停进高亮窗口。</span>
      </div>
    </div>
  `;
  const scoreNode = stage.querySelector("#rhythm-score");
  const roundNode = stage.querySelector("#rhythm-round");
  const missNode = stage.querySelector("#rhythm-miss");
  const pointerNode = stage.querySelector("#rhythm-pointer");
  const button = stage.querySelector("#rhythm-hit");
  const note = stage.querySelector("#rhythm-note");
  ...
  button.addEventListener("click", () => {
    if (locked) return;
    locked = true;
    const hit = marker >= 42 && marker <= 58;
    if (hit) {
      score += 1;
      scoreNode.textContent = String(score);
      note.textContent = "命中节拍。";
    } else {
      misses += 1;
      missNode.textContent = String(misses);
      note.textContent = "节拍偏移。";
    }
    roundNode.textContent = String(round);
    if (round >= 5) {
      finish(`节奏完成，命中 ${score} 次。`);
      return;
    }
    round += 1;
...
function playTyping(stage) {
  const prompts = ["NEON", "ORBIT", "PULSE", "GLIDE", "ANCHOR"];
  let index = 0;
  let score = 0;
  let timeLeft = 35;
  let finished = false;
  stage.innerHTML = `
    <div class="play-stage">
      ...
      <div class="score-box">
        请快速输入：<strong id="typing-word"></strong>
      </div>
      <div class="stage-toolbar">
        <input class="name-input" id="typing-input" type="text" maxlength="18" placeholder="输入关键词后回车">
        <button class="button primary" id="typing-submit" type="button">提交</button>
      </div>
      <div class="log-box" id="typing-log">准备开始输入挑战。</div>
    </div>
  `;
  ...
  const button = stage.querySelector("#typing-submit");
  ...
  const renderPrompt = () => {
    ...
    wordNode.textContent = prompts[index];
    input.value = "";
    input.focus();
  };
  ...
  const submit = () => {
    ...
    if (!value) return;
    if (value === prompts[index]) {
      score += 1;
      scoreNode.textContent = String(score);
      log.textContent = `正确：${prompts[index]}`;
    } else {
      log.textContent = `错误，正确答案是 ${prompts[index]}。`;
    }
    index += 1;
    if (index >= prompts.length) {
      finish(`输入挑战结束，得分 ${score}。`);
      return;
    }
    renderPrompt();
  };
...
function playSequence(stage) {
  ...
  stage.innerHTML = `
    <div class="play-stage">
      <div class="status-line">
        ...
        <span class="status-pill">长度<strong id="sequence-length">0</strong></span>
      </div>
      <div class="score-box" id="sequence-display">等待序列生成。</div>
      <div class="sequence-grid" id="sequence-grid"></div>
      <div class="log-box" id="sequence-log">记住上方顺序后，再按下方按钮复现。</div>
    </div>
  `;
  const roundNode = stage.querySelector("#sequence-round");
  const lengthNode = stage.querySelector("#sequence-length");
  const display = stage.querySelector("#sequence-display");
  const grid = stage.querySelector("#sequence-grid");
  ...
  const revealSequence = () => {
    ...
    let revealIndex = 0;
    const step = () => {
      display.textContent = sequence.slice(0, revealIndex + 1).map((item) => colors[item]).join(" → ");
      if (revealIndex >= sequence.length - 1) {
        timers.push(window.setTimeout(() => {
          accepting = true;
          display.textContent = "请复现序列";
          log.textContent = "按下方按钮复现颜色顺序。";
        }, 450));
        return;
      }
      timers.push(window.setTimeout(() => {
  ...
  grid.innerHTML = colors.map((label, index) => `
    <button class="sequence-button" data-step="${index}" type="button">${label}</button>
  `).join("");

  grid.addEventListener("click", (event) => {
    const button = event.target.closest(".sequence-button");
    if (!button || !accepting) return;
    const stepIndex = Number(button.dataset.step);
    if (stepIndex !== sequence[inputIndex]) {
      finish(`顺序错误，正确序列是 ${sequence.map((item) => colors[item]).join(" → ")}`);
      return;
    }
    inputIndex += 1;
    log.textContent = `已完成 ${inputIndex}/${sequence.length}`;
...
function playDefense(stage) {
  ...
  stage.innerHTML = `
    <div class="play-stage">
      <div class="status-line">
        <span class="status-pill">击退<strong id="defense-score">${score}</strong></span>
        <span class="status-pill">波次<strong id="defense-round">${round}</strong>/5</span>
        <span class="status-pill">护盾<strong id="defense-shield">${shields}</strong></span>
      </div>
      <div class="defense-board" id="defense-board"></div>
      <div class="stage-toolbar" id="defense-actions"></div>
      <div class="log-box" id="defense-log">阻止敌军穿过终点线。</div>
    </div>
  `;
  ...
  const board = stage.querySelector("#defense-board");
  const actions = stage.querySelector("#defense-actions");
  const log = stage.querySelector("#defense-log");

  const render = () => {
    board.innerHTML = lanes.map((lane, index) => `
      <div class="defense-lane${index === activeLane ? " active" : ""}">
        <strong>${lane}</strong>
        <span>${index === activeLane ? "敌军来袭" : "待命"}</span>
      </div>
    `).join("");
    actions.innerHTML = lanes.map((lane, index) => `
      <button class="button secondary" data-lane="${index}" type="button">守住${lane}</button>
    `).join("");
    actions.querySelectorAll("button").forEach((button) => {
      button.addEventListener("click", () => {
        if (locked) return;
        ...
        if (lane === activeLane) {
          score += 1;
          scoreNode.textContent = String(score);
          log.textContent = "防守成功。";
        } else {
          shields -= 1;
          shieldNode.textContent = String(shields);
          log.textContent = "防守失误。";
          if (shields <= 0) {
            finish(`防线失守，成功击退 ${score} 波。`);
            return;
          }
        }
        round += 1;
        if (round > 5) {
          finish(`防守完成，成功击退 ${score} 波。`);
          return;
        }
        window.setTimeout(nextWave, 450);
      });
    });
  };
  ...
  const nextWave = () => {
    locked = false;
    activeLane = Math.floor(Math.random() * lanes.length);
    roundNode.textContent = String(round);
    render();
    log.textContent = `第 ${round} 波来袭。`;
    waveTimer = window.setTimeout(() => {
      if (locked) return;
      shields -= 1;
      shieldNode.textContent = String(shields);
      log.textContent = "反应太慢，防线被突破。";
      if (shields <= 0) {
        finish(`防线失守，成功击退 ${score} 波。`);
        return;
      }
...
function playGravity(stage) {
  ...
  stage.innerHTML = `
    <div class="play-stage">
      <div class="status-line">
        <span class="status-pill">步数<strong id="gravity-moves">${moves}</strong></span>
        <span class="status-pill">护壳<strong id="gravity-hull">${hull}</strong></span>
        <span class="status-pill">重力<strong id="gravity-direction">${directions[gravityIndex].label}</strong></span>
      </div>
      <div class="gravity-board" id="gravity-board"></div>
      <div class="stage-toolbar">
        <button class="button secondary" id="gravity-flip" type="button">翻转重力</button>
        <button class="button primary" id="gravity-step" type="button">推进一步</button>
      </div>
      <div class="log-box" id="gravity-log">翻转重力并把方块送进停靠港。</div>
    </div>
  `;
  const board = stage.querySelector("#gravity-board");
  const movesNode = stage.querySelector("#gravity-moves");
  const hullNode = stage.querySelector("#gravity-hull");
  const directionNode = stage.querySelector("#gravity-direction");
  const log = stage.querySelector("#gravity-log");
  const flipButton = stage.querySelector("#gravity-flip");
  const stepButton = stage.querySelector("#gravity-step");

  const render = () => {
    directionNode.textContent = directions[gravityIndex].label;
    movesNode.textContent = String(moves);
    hullNode.textContent = String(hull);
    board.innerHTML = layout.map((cells, rowIndex) => cells.map((cell, colIndex) => {
      const classes = ["gravity-cell"];
      if (cell === 1) classes.push("wall");
      if (rowIndex === goal.row && colIndex === goal.col) classes.push("goal");
      if (rowIndex === player.row && colIndex === player.col) classes.push("player");
      return `<div class="${classes.join(" ")}"></div>`;
  ...
  const applyMove = () => {
    const dir = directions[gravityIndex];
    const next = { row: player.row + dir.row, col: player.col + dir.col };
    moves += 1;
    if (next.row < 0 || next.row >= size || next.col < 0 || next.col >= size || layout[next.row][next.col] === 1) {
      hull -= 1;
      log.textContent = "撞上障碍，护壳受损。";
      if (hull <= 0) {
        render();
        finish("停靠失败。");
        return;
    ...
    if (player.row === goal.row && player.col === goal.col) {
      render();
      finish(`成功停靠，用了 ${moves} 步。`);
      return;
    }
    log.textContent = "推进成功。";
    render();
  };

  flipButton.addEventListener("click", () => {
    gravityIndex = (gravityIndex + 1) % directions.length;
    render();
  });

  stepButton.addEventListener("click", applyMove);
  render();
  return () => {};
}
